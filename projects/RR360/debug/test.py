...
def main():
    ...
    from projects.RR360.debug.fps_hook import FPSHook  # noqa F401

    if args.mongo is not None:
        FPSHook_CONFIG = dict(type='FPSHook', mongo=args.mongo)
        if hasattr(cfg, 'custom_hooks'):
            ...
            if not FPSHookExists:
                cfg.custom_hooks.append(FPSHook_CONFIG)
        else:
            cfg.custom_hooks = [FPSHook_CONFIG]
